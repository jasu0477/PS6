...
const Navbar = () => {
  return (
    <nav className="bg-base-200 p-7 flex items-center justify-between shadow-lg h-24">
      {/* Logo / Company Name */}
      <div className="text-4xl text-primary font-bold cursor-pointer">HandyGo</div>
        ...
        {["Home", "Notifications", "Profile", "Help"].map((item, index) => (
          <a
            ...
            className="hover:text-primary transition-all duration-300 relative 
                       after:content-[''] after:block after:w-full after:h-[1.5px] after:bg-primary 
      ...
      {/* Logout Button */}
      <button className="btn btn-error btn-md px-6 text-lg transition-all duration-300 hover:bg-red-500">
        Logout
      </button>
    </nav>
  );
